...
class OverlapPatchEmbed3D(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=1, embed_dim=768):
        ...
        self.img_size = img_size
        self.patch_size = patch_size
        self.D, self.H, self.W = img_size[0] // stride, img_size[1] // stride, img_size[2] // stride
...
class PyramidVisionTransformerV223D(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], num_stages=4, linear=False):
        ...
        for i in range(num_stages):
            patch_embed = OverlapPatchEmbed(img_size=img_size if i == 0 else img_size // (2 ** (i + 1)),
                                            ...
                                            in_chans=in_chans if i == 0 else embed_dims[i - 1],
                                            embed_dim=embed_dims[i])
            patch_embed3 = OverlapPatchEmbed3D(img_size=(100,50,50) if i == 0 else (100// (2 ** (i + 1)),50// (2 ** (i + 1)),50// (2 ** (i + 1))),
                                            patch_size=7 if i == 0 else 3,
            ...
            setattr(self, f"patch_embed{i + 1}", patch_embed)
            setattr(self, f"patch_embed3{i + 1}", patch_embed3)
            setattr(self, f"block{i + 1}", block)
            setattr(self, f"norm{i + 1}", norm)
    ...
    def _init_weights(self, m):
        ...
        #     fan_out = m.kernel_size[0] * m.kernel_size[1] * m.kernel_size[3] * m.out_channels
        #     fan_out //= m.groups
        #     m.weight.data.normal_(0, math.sqrt(2.0 / fan_out))
        #     if m.bias is not None:
        #         m.bias.data.zero_()
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            patch_embed = getattr(self, f"patch_embed{i + 1}")
            patch_embed3 = getattr(self, f"patch_embed3{i + 1}")
            block = getattr(self, f"block{i + 1}")
            norm = getattr(self, f"norm{i + 1}")
            if len(x.shape)==4:
                x, H, W = patch_embed(x)
...
@register_model
def pvt_v2_b1_23D(pretrained=False, **kwargs):
    model = PyramidVisionTransformerV223D(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
        **kwargs)
    model.default_cfg = _cfg()
